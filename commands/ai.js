...
module.exports = {
  ...
  async execute(senderId, args) {
    ...
    if (!query) {
      ...
      const formattedMessage = useFontFormatting ? formatresponse(defaultMessage, selectedFont) : defaultMessage;
      return await sendMessage(senderId, { text: formattedMessage }, pageAccessToken);
    }

    if (query === "jsnekwksnekanswjkw" || query === "jsjwjegeiwjsjkwjsjs") {
      const jokeMessage = "Baliw HAHAAH";
      const formattedMessage = useFontFormatting ? formatresponse(jokeMessage, selectedFont) : jokeMessage;
      return await sendMessage(senderId, { text: formattedMessage }, pageAccessToken);
    }
...
const handleChatResponse = async (senderId, input, pageAccessToken) => {
  ...
  try {
    ...
    const formattedResponse = useFontFormatting ? formatresponse(response, selectedFont) : response;
    ...
    const formattedMessage = useFontFormatting ? formatresponse(errorMessage, selectedFont) : errorMessage;
...
function formatresponse(responsetext, fontNumber) {
  const fonts = {
    0: {
      ' ': ' ',
  ...
  const selectedMapping = fonts[fontNumber];
  return responsetext.split('').map(char => selectedMapping[char] || char).join('');
}
